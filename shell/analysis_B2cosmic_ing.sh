...
#cmd=`printf "%s/TwoDimRecon -r %d -s %d -c" ${ANALYSIS_DIR} ${RUNID} ${SRUNID}`
#echo -e "---------\n ${cmd}\n"; eval $cmd

#cmd=`printf "%s/ThreeDimRecon -r %d -s %d -c -m 0" ${ANALYSIS_DIR} ${RUNID} ${SRUNID}`
#echo -e "---------\n ${cmd}\n"; eval $cmd
#cmd=`printf "%s/ThreeDimRecon -r %d -s %d -c -m 1" ${ANALYSIS_DIR} ${RUNID} ${SRUNID}`
#echo -e "---------\n ${cmd}\n"; eval $cmd
...
#P.E
thres_pe=4
cmd=`printf "%s/B2pe -r %d -s %d -m1 -t%d" ${ANALYSIS_DIR} ${RUNID} ${SRUNID} ${thres_pe}`
echo -e "---------\n ${cmd}\n"; eval $cmd

#P.E in scinti
thres_pe=0
cmd=`printf "%s/B2pe_Scinti -r %d -s %d -m1 -t%d" ${ANALYSIS_DIR} ${RUNID} ${SRUNID} ${thres_pe}`
echo -e "---------\n ${cmd}\n"; eval $cmd

#####Hit Efficiency 2 (Distribution in Scinti)
#for i in `seq 4 4`; do
#  thres_pe=$i
#  cmd=`printf "%s/HitEfficiency_2 -r %d -s %d -m1 -t%d" ${ANALYSIS_DIR} ${RUNID} ${SRUNID} ${thres_pe}`
#  echo -e "---------\n ${cmd}\n"; eval $cmd
#done
