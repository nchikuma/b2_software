...
OUTPUT_DIR2="/home/t2k/nchikuma/b2_data2/mc_neut4"

#OUTPUT_DIR2_0="/home/t2k/nchikuma/b2_data2/reweight/rfg"
#OUTPUT_DIR2_1="/home/t2k/nchikuma/b2_data2/reweight/rpa"
OUTPUT_DIR2_0="/home/t2k/nchikuma/b2_data2/mc_neut2/rfg"
OUTPUT_DIR2_1="/home/t2k/nchikuma/b2_data2/mc_neut2/rpa"
...
PLOTFILE1=`printf "%s/h2o/numubar/ingrid_%08d_%04d_plot.root" $OUTPUT_DIR2 $RUNID $SRUNID`
PLOTFILE2=`printf "%s/h2o/numu/ingrid_%08d_%04d_plot.root"    $OUTPUT_DIR2 $RUNID $SRUNID`
PLOTFILE3=`printf "%s/ch/numubar/ingrid_%08d_%04d_plot.root"  $OUTPUT_DIR2 $RUNID $SRUNID`
PLOTFILE4=`printf "%s/ch/numu/ingrid_%08d_%04d_plot.root"     $OUTPUT_DIR2 $RUNID $SRUNID`
PLOTFILE5=`printf "%s/fe/numubar/ingrid_%08d_%04d_plot.root"  $OUTPUT_DIR2 $RUNID $SRUNID`
PLOTFILE6=`printf "%s/fe/numu/ingrid_%08d_%04d_plot.root"     $OUTPUT_DIR2 $RUNID $SRUNID`
...
then
  ...
  ##Plot 
  cmd=`printf "%s/B2Plot -i %s -o %s -m 7 -t %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE1} ${PLOTFILE1} ${TUNEFILE} ${OPT_RATIO} ${OPT_ONLYNSEL}`
  echo -e "---------\n ${cmd}\n"; eval $cmd
  cmd=`printf "%s/B2Plot -i %s -o %s -m 7 -t %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE2} ${PLOTFILE2} ${TUNEFILE} ${OPT_RATIO} ${OPT_ONLYNSEL}`
  echo -e "---------\n ${cmd}\n"; eval $cmd
  cmd=`printf "%s/B2Plot -i %s -o %s -m 8 -t %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE3} ${PLOTFILE3} ${TUNEFILE} ${OPT_RATIO} ${OPT_ONLYNSEL}`
  echo -e "---------\n ${cmd}\n"; eval $cmd
  cmd=`printf "%s/B2Plot -i %s -o %s -m 8 -t %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE4} ${PLOTFILE4} ${TUNEFILE} ${OPT_RATIO} ${OPT_ONLYNSEL}`
  echo -e "---------\n ${cmd}\n"; eval $cmd
...
then
  ...
  do
    ...
    PLOTFILE1=`printf "%s/h2o/numubar/ingrid_%08d_%04d_plot_%d.root" $OUTPUT_DIR2 $RUNID $SRUNID $l`
    PLOTFILE2=`printf "%s/h2o/numu/ingrid_%08d_%04d_plot_%d.root"    $OUTPUT_DIR2 $RUNID $SRUNID $l`
    PLOTFILE3=`printf "%s/ch/numubar/ingrid_%08d_%04d_plot_%d.root"  $OUTPUT_DIR2 $RUNID $SRUNID $l`
    PLOTFILE4=`printf "%s/ch/numu/ingrid_%08d_%04d_plot_%d.root"     $OUTPUT_DIR2 $RUNID $SRUNID $l`
    ...
    cmd=`printf "%s/B2Plot -i %s -o %s -m 7 -t %s -w %d -r %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE1} ${PLOTFILE1} ${TUNEFILE} ${dial} ${REWEIGHTFILE1} ${OPT_RATIO} ${OPTPLOT}`
    echo -e "---------\n ${cmd}\n"; eval $cmd
    cmd=`printf "%s/B2Plot -i %s -o %s -m 7 -t %s -w %d -r %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE2} ${PLOTFILE2} ${TUNEFILE} ${dial} ${REWEIGHTFILE2} ${OPT_RATIO} ${OPTPLOT}`
    echo -e "---------\n ${cmd}\n"; eval $cmd
    cmd=`printf "%s/B2Plot -i %s -o %s -m 8 -t %s -w %d -r %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE3} ${PLOTFILE3} ${TUNEFILE} ${dial} ${REWEIGHTFILE3} ${OPT_RATIO} ${OPTPLOT}`
    echo -e "---------\n ${cmd}\n"; eval $cmd
    cmd=`printf "%s/B2Plot -i %s -o %s -m 8 -t %s -w %d -r %s %s %s -a" ${ANALYSIS_DIR} ${ANAFILE4} ${PLOTFILE4} ${TUNEFILE} ${dial} ${REWEIGHTFILE4} ${OPT_RATIO} ${OPTPLOT}`
    echo -e "---------\n ${cmd}\n"; eval $cmd
