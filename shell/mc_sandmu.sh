...
if [ $# -ne 2 ]; then
  ...
  echo "==============================================="
  echo "Usage:                                         "
  echo "  ./mc_sandmu.sh <run_id> <srun_id>"
  echo "==============================================="
  exit
fi
...
OUTPUTFILE=`printf "%s/ingrid_%08d_%04d_calib.root" $OUTPUT_SANDMU_DIR $RUNID $SRUNID`
RECONFILE=`printf "%s/ingrid_%08d_%04d_recon.root" $OUTPUT_SANDMU_DIR $RUNID $SRUNID`
...
#MC Job
#sand muon
cmd=`printf "%s/bin/Linux-g++/b2mc -m7 -f11 -o %s -e 1000" $MC_DIR $OUTPUTFILE`
echo -e "---------\n ${cmd}\n"; eval $cmd
...
#  thres_nhit=$i
#  cmd=`printf "%s/TrackEfficiency -r %d -s %d -m2 -t%d" ${ANALYSIS_DIR} ${RUNID} ${SRUNID} ${thres_nhit}`
#  echo -e "---------\n ${cmd}\n"; eval $cmd
#done
...
#for i in `seq 0 7`; do
#  thres_pe=$i
#  cmd=`printf "%s/HitEfficiency -r %d -s %d -m2 -t%d" ${ANALYSIS_DIR} ${RUNID} ${SRUNID} ${thres_pe}`
#  echo -e "---------\n ${cmd}\n"; eval $cmd
#done
...
#for i in `seq 4 4`; do
#  thres_pe=$i
#  cmd=`printf "%s/HitEfficiency_2 -r %d -s %d -m2 -t%d" ${ANALYSIS_DIR} ${RUNID} ${SRUNID} ${thres_pe}`
#  echo -e "---------\n ${cmd}\n"; eval $cmd
#done

##P.E in scinti
#thres_pe=0
#cmd=`printf "%s/B2pe_Scinti -r %d -s %d -m2 -t%d" ${ANALYSIS_DIR} ${RUNID} ${SRUNID} ${thres_pe}`
#echo -e "---------\n ${cmd}\n"; eval $cmd
