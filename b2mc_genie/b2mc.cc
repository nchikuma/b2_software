...
int main(int argc, char** argv) 
{
  ...
  //  Setup for GENIE
  // ================
  TFile *fgenie = new TFile(neutfile,"read");
  //TTree *tgenie = (TTree*)fgenie->Get("gst");
  TTree *tgenie = (TTree*)fgenie->Get("gtree");
  NumberOfEvent = tgenie->GetEntries();
  G4cout << NumberOfEvent << endl;
  ...
  RunAction * rac = new RunAction(output,tgenie);
  ...
  runManager-> SetUserAction(new PrimaryGeneratorAction(rac, evt, nd, flav));
