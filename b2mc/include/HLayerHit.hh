...
class HLayerHit : public G4VHit 
{
public:
  //HLayerHit(G4int, G4int, G4int, G4double, G4double, G4ThreeVector, G4double);
  HLayerHit(G4int, G4int, G4int, G4double, G4double, G4ThreeVector, G4double,DetectorDimension*);
  HLayerHit(G4int, G4double, G4int);
  HLayerHit(G4int, G4double);
  
  ~HLayerHit();
  HLayerHit(const HLayerHit &right);
  const HLayerHit& operator=(const HLayerHit &right);
  G4int operator==(const HLayerHit &right) const;
  G4int CompareID(const HLayerHit right) ;
  G4int CompareP(const HLayerHit right);
  G4int isFaster(const HLayerHit right);
  G4int LargerEdep(const HLayerHit right);
...
typedef G4THitsCollection<HLayerHit> HLayerHitsCollection;

extern G4Allocator<HLayerHit> HLayerHitAllocator; 

inline void* HLayerHit::operator new(size_t)
{
  void *aHit;
  aHit = (void *) HLayerHitAllocator.MallocSingle();
  return aHit;
}

inline void HLayerHit::operator delete(void *aHit)
{
  HLayerHitAllocator.FreeSingle((HLayerHit*) aHit);
}
