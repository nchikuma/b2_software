  ...
  while((opt = getopt(argc,argv, "f:i:o:rh")) !=-1 ){
    switch(opt){
      ...
      case 'h':
        ...
        cout <<"Usage:./Decoder [-h] [-f input] ..."<<endl;
        cout <<"  -h         : help"<<endl;
        cout <<"  -f (char*) : choose inputfile you want to read(must)"<<endl;
        cout <<"  -i (char*) : choose calibration file(calib_result.xml)."<<endl;
        cout <<"  -o (char*) : choose output directory. (default=WAGASCI_DECODEDIR)"<<endl;
        cout <<"  -r         : overwrite mode "<<endl;
        cout <<" (ex.):./Decoder -rf test_dif_1_1_1.raw"<<endl;
        exit(0);
      default:
        cout << "See the usage:" << endl;
        cout << argv[0] << " -h" <<endl;
        exit(0);
    }
  }

  if(inputFileName==""||calibFileName==""){
    cout << "!!ERROR!! please input filename, and calibration file." <<endl;
    cout << "See the usage:" << endl;
    cout << argv[0] << " -h" <<endl;
    exit(0);
  }
...
void ReadFile(string inputFileName, string calibFileName, bool overwrite, unsigned int maxEvt,
  ...
  cout << " =====  CALIB FILE       :" << calibFileName      << "  =====" << endl;
#endif
  ...
  getcalib->Get_TdcCoeff(ndif,tdc_slope,tdc_intcpt);
  ...
  info->Branch("start_time"   ,&inform.start_time   ,Form("start_time/L"));
  info->Branch("stop_time"    ,&inform.stop_time    ,Form("stop_time/L"));
  info->Branch("nb_data_pkts" ,&inform.nb_data_pkts ,Form("nb_data_pkts/L"));
  info->Branch("nb_lost_pkts" ,&inform.nb_lost_pkts ,Form("nb_lost_pkts/L"));
  inform.start_time   = v_log[0]; 
  inform.stop_time    = v_log[1];
  inform.nb_data_pkts = v_log[2];
  inform.nb_lost_pkts = v_log[3];
  info->Fill();
  info->Write();
