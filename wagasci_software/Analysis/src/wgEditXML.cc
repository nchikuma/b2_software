...
//**********************************************************************
void wgEditXML::GetLog(string& filename,vector<long>& v){
  string str("");
  string target("");
  v.clear();
  v.resize(4);
...
//**********************************************************************
void wgEditXML::DQTime_Make(string& filename){
  xml = new XMLDocument();
  XMLDeclaration* decl = xml->NewDeclaration();
  xml->InsertEndChild(decl);
  ...
  data = xml->NewElement("data");
  xml->InsertEndChild(data);
  start_time = xml->NewElement(Form("start_time"));    
  data->InsertEndChild(start_time);
  stop_time = xml->NewElement(Form("stop_time"));    
  data->InsertEndChild(stop_time);
...
//**********************************************************************
void wgEditXML::DQTime_SetValue(string& name,int value, int mode=0){
  XMLElement* data = xml->FirstChildElement("data");
  XMLElement* target = data->FirstChildElement(name.c_str());
...
//**********************************************************************
int  wgEditXML::DQTime_GetValue(string& name){
  XMLElement* data = xml->FirstChildElement("data");
  XMLElement* target = data->FirstChildElement(name.c_str());
  if(target){
    string value = target->GetText();
